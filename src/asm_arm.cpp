...
ASM::AS_reg *ASM::AS_Reg_Sp()
{
    return new AS_reg(-1, -1);
}
ASM::AS_reg *ASM::AS_Reg_Imm(int imm)
{
    return new AS_reg(-3, imm);
}
ASM::AS_reg *ASM::AS_Reg_OnStack(int offset)
{
    return new AS_reg(-1, offset);
}

ASM::AS_reg *ASM::AS_Reg_Operand(AS_operand *oper)
{
    switch (oper->kind)
    {
    case OperandKind::ICONST:
    {
        int instant = oper->u.ICONST;
        return new AS_reg(-3, instant);
    }
    case OperandKind::TEMP:
    {
        return new AS_reg(oper->u.TEMP->num, 0);
        break;
    }
