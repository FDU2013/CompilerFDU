...
Graph<L_block*>& Bg_graph() { return RA_bg; }
unordered_map<Temp_label*, L_block*>& Bg_block_env() { return block_env; }
...
static void DFS(Node<L_block*>* r, Graph<L_block*>& bg) {
  ...
  for (auto succ_count : r->succs) {
    ...
    if (succ_node->color != 233) DFS(succ_node, bg);
  }
}
...
void SingleSourceGraph(Node<L_block*>* r, Graph<L_block*>& bg, L_func* fun,
  ...
  assert(fun->blocks.size() == bg.nodecount);
  for (auto it = fun->blocks.begin(); it != fun->blocks.end(); count_key++) {
    if (bg.mynodes[count_key]->color != 233) {
      node_deprecated.insert(*it);
